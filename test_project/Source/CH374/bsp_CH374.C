...
/* 
	...
	FSMC_NE4 = 0; FSMC_A22 = 0时，选中CH374T。
	由于是16bit模式，CPU的FSMC_A22口线实际对应的地址线为A23。
	
    FSMC_A18 和 CH374T的A0连接，用于选择是命令端口还是数据端口，同理，FSMC_A18实际对应地址线A19
		80000

	CH374的文件库需要借助一个全局的结构变量进行文件操作，函数的封装性不强。我们将
	部分函数进行了重新封装。
*/
#define CH374_BASE_ADDR	0x6C000000	
#define CH374_IDX_PORT	(*((volatile uint16_t *) (CH374_BASE_ADDR + 0x80000)))	/* CH374索引端口的I/O地址 */
...
/* 仅允许本文件内调用的函数声明 */
static void ch374_CfgGPIO(void);
...
static void ch374_CfgGPIO(void)
{
	...
	/* 设置 PD.00(D2), PD.01(D3), PD.04(NOE), PD.05(NWE),PD.13(FSMC_A18), PD.14(D0), PD.15(D1) 为复用推挽输出 */
	...
	/* CH374的中断输出引脚连接到PE2, PE2配置为输入 */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOE, &GPIO_InitStructure);	/* PA0 */
...
/* 向索引端口写入索引地址 */
//#define	Write374Index( a )	{ CH374_IDX_PORT = a; }	
...
/* 向数据端口写入数据,索引地址自动加1 */
//#define	Write374Data( d )	{ CH374_DAT_PORT = d & 0xFF; }	
void Write374Data( UINT8 mData )
{
	CH374_DAT_PORT = mData;
}

/* 从数据端口读出数据,索引地址自动加1 */
//#define	Read374Data( )		( (UINT8)CH374_DAT_PORT )
UINT8 Read374Data( void )
{
    return (CH374_DAT_PORT);
}

/* 从索引端口读出数据，索引地址不变，适用于[读出->修改->写回]操作 */
//#define	Read374Data0( )		( (UINT8)CH374_IDX_PORT )		
UINT8 Read374Data0( void )
{
